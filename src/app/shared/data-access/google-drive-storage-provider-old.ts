...
export class GoogleDriveStorageProviderOld {
    ...
    public appFolderId$ = new BehaviorSubject<string | null>(null);
    public appFolderContents$ = this.appFolderId$.pipe(
        mergeMap(folderId => folderId ? this.getListOfFiles(folderId) : EMPTY)
    );

    appStorage$ = this.appFolderId$.pipe(
        mergeMap(folderId => folderId ? this.getFileContents<AppStorage>(folderId) : EMPTY)
    );

    constructor() {

        this.appData$
            .subscribe(data => this.appFolderId$.next(data.storageFileId));
    ...
    getPeriodStorage(): Observable<AppStorage> {
        throw new Error("Method not implemented.");
        // return this.getListOfFiles(spaces)
    }
